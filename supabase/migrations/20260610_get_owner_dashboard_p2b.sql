-- get_owner_dashboard — P2b: include member id in ghost preview
--
-- The dashboard's "At-risk members" preview now opens the MemberDrawer in
-- place (instead of routing to the Members page), so each preview row needs
-- the member's id. CREATE OR REPLACE supersedes 20260610_get_owner_dashboard_p2.sql;
-- only the ghost CTE + preview select changed (added id). Guard + grants
