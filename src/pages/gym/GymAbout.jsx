...
export default function GymAbout() {
  ...
  const heroImg     = content?.about_page_image || null
  const heroAlign   = content?.about_page_align || 'left'
  ...
  return (
    <div style={{ background: 'var(--gym-bg)' }}>
      ...
      {/* ── Page Hero ── */}
      {!hidden.includes('page_hero_about') && (
      ...
      >
        <div className="max-w-6xl mx-auto px-6" style={{ paddingBlock: "var(--gym-section-py)" }}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <motion.div variants={slideInRight} className="relative overflow-hidden" style={{ aspectRatio: '4/3', borderRadius: 'var(--gym-card-radius)' }}>
      ...
      {/* ── Why Choose Us ── */}
      {!hidden.includes('why_us_content') && (
        <motion.section
          variants={staggerContainer} initial="hidden" whileInView="visible" viewport={scrollViewport}
          ...
          <div className="max-w-6xl mx-auto px-6" style={{ paddingBlock: "var(--gym-section-py)" }}>
            <motion.div variants={fadeUp} className="mb-14 text-center">
              ...
              <h2 className="font-display text-white tracking-wide" style={{ fontSize: 'var(--gym-h2-size)' }}>
                {(content?.why_us_heading || `THE ${gym.name} DIFFERENCE`).toUpperCase()}
              </h2>
            </motion.div>
      ...
      {/* ── Vision & Mission ── */}
      {!hidden.includes('vision_mission') && (
        <motion.section
          variants={staggerContainer} initial="hidden" whileInView="visible" viewport={scrollViewport}
        >
          <div className="max-w-6xl mx-auto px-6" style={{ paddingBlock: "var(--gym-section-py)" }}>
            <motion.div variants={fadeUp} className="mb-12 text-center">
              <p className="text-xs font-bold tracking-[0.25em] uppercase mb-3 font-sans" style={{ color: 'var(--gym-primary)' }}>
                {content?.vision_section_label || 'Purpose'}
              </p>
              <h2 className="font-display text-white tracking-wide" style={{ fontSize: 'var(--gym-h2-size)' }}>
                {(content?.vision_section_heading || 'VISION & MISSION').toUpperCase()}
              </h2>
            </motion.div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[{ label: visionMission[0].label, text: visionText }, { label: visionMission[1].label, text: missionText }].map((item, i) => (
                <motion.div
                  ...
                  style={{ border: '1px solid var(--gym-border)', background: 'var(--gym-card)', borderRadius: 'var(--gym-card-radius)', boxShadow: 'var(--gym-shadow)' }}
                >
        ...
        <div className="max-w-6xl mx-auto px-6 py-16">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-0">
            {displayStats.map((stat, i) => (
              <motion.div key={i} variants={fadeUp} className="flex flex-col items-center py-8 relative">
      ...
      {!hidden.includes('stats_about')   && <StatsSection defaults={defaults} content={content} />}

      {/* ── CTA ── */}
      {!hidden.includes('cta_about') && (
