...
export default function GymContact() {
  ...
  const heroImg   = content?.contact_page_image || null
  const heroAlign = content?.contact_page_align || 'left'
  ...
  return (
    <div style={{ background: 'var(--gym-bg)' }}>
      ...
      {/* ── Page Hero ── */}
        {!hidden.includes('page_hero_contact') && (
                ...
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    {[
                      ...
                      <div key={field.id}>
                        ...
                        <input type={field.type} placeholder={field.placeholder} value={form[field.id]}
                          ...
                          className="w-full px-5 py-4 text-sm font-sans text-white placeholder-white/20 outline-none transition-all duration-300"
                          style={{ background: 'var(--gym-card)', border: '1px solid var(--gym-border)', borderRadius: 'var(--gym-card-radius)' }}
                    ...
                    <div>
                      ...
                      <textarea placeholder="How can we help you?" rows={4} value={form.message}
                        onChange={e => setForm({ ...form, message: e.target.value })} required
                        className="w-full px-5 py-4 text-sm font-sans text-white placeholder-white/20 outline-none resize-none transition-all duration-300"
                        style={{ background: 'var(--gym-card)', border: '1px solid var(--gym-border)', borderRadius: 'var(--gym-card-radius)' }}
                        onFocus={e => e.target.style.borderColor = 'var(--gym-primary)'}
                        onBlur={e => e.target.style.borderColor = 'var(--gym-border)'}
                      />
              ...
              <motion.div variants={slideInRight} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {hours.map((h, i) => (
                      <div key={i} className="flex items-center justify-between" style={{ borderBottom: i < hours.length - 1 ? '1px solid var(--gym-border)' : 'none', paddingBottom: i < hours.length - 1 ? '1rem' : 0 }}>
                        ...
                        <span className="text-sm font-semibold font-sans" style={{ color: 'var(--gym-primary)' }}>{h.time}</span>
                      </div>
                    ))}
                  </div>
                </div>
                {(gym.phone || gym.email || gym.address || gym.city) && (
                  <div className="p-7" style={{ background: 'var(--gym-card)', border: '1px solid var(--gym-border)', borderRadius: 'var(--gym-card-radius)' }}>
                    ...
                    {(gym.address || gym.city) && (
                      <div className="mt-5 overflow-hidden" style={{ height: '200px', borderRadius: 'var(--gym-card-radius)' }}>
