    ...
    <div className="space-y-6 max-w-[1200px]">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        {Array(3).fill(0).map((_, i) => (
          <div key={i} className="space-y-1.5"><Sk h={12} w={110} /><Sk h={42} r={8} /></div>
        ))}
        <div className="space-y-1.5"><Sk h={12} w={80} /><Sk h={80} r={8} /></div>
        <Sk h={38} w={100} r={8} />
      </div>
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        <Sk h={18} w={160} />
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {Array(6).fill(0).map((_, i) => <Sk key={i} h={32} w={32} r={99} />)}
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={handleSave} className="space-y-5">
        ...
        {/* Logo upload */}
        <Field label="Gym Logo" hint="Compressed & converted to WebP automatically. Replaces any existing logo.">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-1">
            {[
              ...
              <button key={m.value} type="button" onClick={() => setThemeMode(m.value)}
                className={`flex-1 flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all cursor-pointer text-left ${themeMode === m.value ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
    ...
    <div className="flex items-center gap-3 px-4 py-3.5 rounded-xl border border-gray-200 bg-white hover:border-gray-300 transition-colors">
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {badge && <span className="text-[10px] px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded font-bold">{badge}</span>}
        </div>
        ...
        <div className="flex flex-wrap gap-2 mt-1">
          {PROG_CATEGORIES.map(c => (
            <button key={c} type="button" onClick={() => setCategory(c)}
              className={`px-3 py-1.5 text-xs font-bold rounded-lg border cursor-pointer transition-colors ${category === c ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-200 hover:border-indigo-300'}`}>
              {c}
            </button>
          ))}
        </div>
...
export default function StarterWebsitePage() {
  ...
  if (loading) return <StarterWebsiteSkeleton />
